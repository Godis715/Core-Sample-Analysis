...
def _describe_params_isCorrect(params):
    return len(ALLOWED_PARAMS - set(params)) == 0 and len(ALLOWED_PARAMS) == len(params)


def decode_archive(path_archive):
    """Decoding zip file with images of sample and file of describe (.json or .xlsx format)"""
    ...
    if format_archive == 'zip':
        ...
        with ZipFile(path_archive) as zip_archive:
            ...
            path_file_describe = f'{root_folder}describe'
            if f'{path_file_describe}.xlsx' in zip_archive.namelist():
                with zip_archive.open(f'{path_file_describe}.xlsx') as file_describe_xlsx:
                    file_bytes = file_describe_xlsx.read()
                    workbook = xlrd.open_workbook(file_contents=file_bytes)
                    sheet = workbook.sheet_by_index(0)
                    header = sheet.row_values(0)
                    if not _describe_params_isCorrect(header):
                        return {'Type': 'Error', 'Message': 'Describe params is not correct!'}
            ...
            elif f'{path_file_describe}.json' in zip_archive.namelist():
                with zip_archive.open(f'{path_file_describe}.json') as file_describe_json:
                    file_bytes = file_describe_json.read()
                    describe_data = json.loads(file_bytes.decode("utf-8"))
                    for describe_sample in describe_data:
                        if not _describe_params_isCorrect(describe_sample.keys()):
                            return {'Type': 'Error', 'Message': 'Describe params is not correct!'}
                        sample = {}
                        for param, value in describe_sample.items():
                ...
                return {'Type': 'Error', 'Message': 'Describe file not exist!'}
    else:
